...
namespace Main
{
    ...
    internal class Window3D : GameWindow
    {
        ...
        private int selectedFaceIndex = 1; // Indexul fetei a carui culoare se va schimba

        private bool modifyColors = false;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            // Seteteaza pe modul poligon ca sa umple automat
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (keyboard[Key.Q] && !keyboard.Equals(lastKeyPress))
            {
                ...
                if (modifyColors)
                {
                    // Modifică culorile feței cuprinse intre valorile minime și maxime pentru canalul de culoare
                    selectedFaceIndex = (selectedFaceIndex + 1) % (objVertices.GetLength(1) / 3);
                    selectedFaceColor = colorVertices[selectedFaceIndex];
                }
            }
        ...
        private void DrawCube()
        {
            ...
            for (int i = 0; i < 35; i = i + 3)
            {
                if (modifyColors)
                {
                    // Folosește o culoare random în locul culorii predefinite
                    GL.Color3(GenerateRandomColor());
                }
                else
                {
                    // Ia valorile RGB din vertex
                    int red = colorVertices[i / 3].R;
                    int green = colorVertices[i / 3].G;
                    int blue = colorVertices[i / 3].B;

                    // Afiseaza Valorile RGB in consola in timp real
                    Console.WriteLine($"RGB: ({red}, {green}, {blue})");

                    // Culori predefinite
                    GL.Color3(Color.FromArgb(red, green, blue));
                }
